...
The remaining gap (acknowledged in §7.3) is that the Lipschitz constants
are certified at discrete grid points, not over the continuous parameter
domain.  The safety margins (>=20% from bound 1) make this gap negligible
in practice.
...
Certificate structure:
  ...
  3. Evaluate |dR/dp| at each grid point via 4th-order Richardson
     extrapolation with all stencil evaluations as certified flint.arb
     intervals -> certified L_p upper bound.
  4. Certificate: max(R_upper) + L_M*(delta_M/2) + L_p*(delta_p/2) < 1.
     All arithmetic in python-flint arb for rigorous interval bounds.
...
print("=" * 80)
print("cert-r-bound-full-arb.py: Certified R(M,p,d) < 1  [Level-1 Arb]")
...
for d in d_vals:
    for p_off in p_offsets:
        ...
        for M_val in M_vals:
            ...
            # Step 2: certified Arb derivatives (L1 — Richardson in Arb arithmetic)
            drM_arb = dR_dM_arb(M_val, p_val, d)
            drp_arb = dR_dp_arb(M_val, p_val, d)

            # Extract certified upper bounds on |dR/dM| and |dR/dp|
...
# ----------------------------------------------------------------
# The key fix over cert-r-bound.py: instead of global L_M x global delta_M
...
#     local_L_M = max(|dR/dM|(M_i), |dR/dM|(M_{i+1}))   [certified Arb upper bound]
...
print()
print("=" * 80)
print("CERTIFICATE COMPUTATION — interval-wise local Lipschitz  [L1 Arb-certified]")
print("=" * 80)
...
print(f"global L_p (certified Arb upper)  : {max_L_p:.8f}")
...
print(f"max L_p (global, certified Arb)   : {max_L_p:.6f}")
...
if certified:
    print(f"\nCERTIFICATE [L1-ARB]: R(M,p,d) < {cb_upper:.4f} < 1")
    ...
    print()
    print("  NOTE (L1 certificate): Lipschitz constants are certified flint.arb upper")
    print("  bounds at discrete grid points; continuity between grid points is assumed")
    print("  (not formally certified).  The >=20% safety margin from bound 1 makes")
    print("  this between-point gap negligible in practice.")
else:
